...
/**
 * origin see javadoc below. This is the original CVParameterSelection from weka renamed to CVParameterSelectionOriginal
 * 
...
public class CVParameterSelectionOriginal extends RandomizableSingleClassifierEnhancer implements Drawable,
	...
	public static void main(final String[] argv) {
		AbstractClassifier.runClassifier(new CVParameterSelectionOriginal(), argv);
	}
	...
	protected void findParamsByCrossValidation(final int depth, final Instances trainData, final Random random)
		...
		if (depth < m_CVParams.size()) {
			...
			for (cvParam.m_ParamValue = cvParam.m_Lower; cvParam.m_ParamValue <= upper; cvParam.m_ParamValue +=
					increment)
			...
			final String[] options = createOptions();
			if (m_Debug) {
			...
			for (int j = 0; j < m_NumFolds; j++) {
				...
				final long testTimeStart = System.currentTimeMillis();
				m_Classifier.buildClassifier(train);
				System.out.println(System.currentTimeMillis() - testTimeStart);
				System.exit(0);
	...
	public Enumeration<Option> listOptions() {
		...
		final Enumeration<?> enu = super.listOptions();
