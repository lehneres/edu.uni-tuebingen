function [B,S] = cdbp(D, k, t, bonus, program)
% CDBP - solves DBP using a C program
% [B,S] = CDBP(D, k, t [, bonus] [, program]), where the inputs are:
% D          the dataset 
% k          size of the decomposition
% t          threshold
% bonus      bonus for covering 1s (optional, defaults to 1)
% program    full program name to use (optional)
%
% and the outputs are:
...
% S          usage matrix.
  ...
  if nargin < 5,
    program = [fileparts(mfilename('fullpath')) '/../asso-solver/asso-solver'];
  end;
  if nargin < 4,
    bonus = 1;
  end;
  ...
  [rows, cols]=size(D);
  prefix = tempname;
  tmpfid = fopen(prefix, 'w');  %% to reserve this filename
  ...
  [status,result] = unix(['tail -' int2str(k) ' ' decompfile ' > ' ...
